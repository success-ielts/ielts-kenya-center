...
function PageEditor({ page, onSave, onClose, busy }: { page:any; onSave:(payload:any)=>Promise<void>; onClose:()=>void; busy:boolean }) {
  ...
  return <div className="ops-detail"><section style={card}>
    ...
    <span className="ops-kicker">PAGE BUILDER</span><h2>{page?.id?'Edit page':'Create page'}</h2>
...
function LearningContentEditor({ item, modules, courses, onSave, onClose, busy }: { item:any; modules:any[]; courses:any[]; onSave:(payload:any)=>Promise<void>; onClose:()=>void; busy:boolean }) {
  const isLesson=Boolean(item?.module_id);
  const [courseId,setCourseId]=useState(item?.course_id||courses[0]?.id||'');
  const [moduleId,setModuleId]=useState(item?.module_id||modules.find(m=>m.course_id===courseId)?.id||'');
  const [title,setTitle]=useState(item?.title||'');
  const [description,setDescription]=useState(item?.description||'');
  const [lessonType,setLessonType]=useState(item?.lesson_type||'lesson');
  ...
  const [content,setContent]=useState(JSON.stringify(item?.content||{blocks:[{type:'heading',text:title||'Lesson'},{type:'paragraph',text:'Add lesson content here.'}]},null,2));
  const courseModules=modules.filter(m=>m.course_id===courseId);
  useEffect(()=>{if(!moduleId && courseModules[0])setModuleId(courseModules[0].id)},[courseId]);
  return <div className="ops-detail"><section style={card}>
    <button className="ops-close" aria-label="Close learning editor" onClick={onClose}><X/></button>
    <span className="ops-kicker">LEARNING BUILDER</span><h2>{isLesson?'Edit lesson':'Edit module'}</h2>
    {!isLesson ? <div className="ops-form-grid"><label>Course<select value={courseId} onChange={e=>setCourseId(e.target.value)}>{courses.map(c=><option key={c.id} value={c.id}>{c.title}</option>)}</select></label><label>Title<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>Description<input value={description} onChange={e=>setDescription(e.target.value)}/></label></div> :
      <><div className="ops-form-grid"><label>Module<select value={moduleId} onChange={e=>setModuleId(e.target.value)}>{modules.map(m=><option key={m.id} value={m.id}>{m.title}</option>)}</select></label><label>Title<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>Type<select value={lessonType} onChange={e=>setLessonType(e.target.value)}>{['lesson','video','exercise','quiz'].map(v=><option key={v} value={v}>{v}</option>)}</select></label><label>Duration (minutes)<input type="number" value={duration} onChange={e=>setDuration(Number(e.target.value))}/></label><label>Published<select value={published?'true':'false'} onChange={e=>setPublished(e.target.value==='true')}><option value="false">Draft</option><option value="true">Published</option></select></label></div><label style={{display:'block',marginTop:14}}>Lesson content JSON<textarea value={content} onChange={e=>setContent(e.target.value)} rows={18} style={{width:'100%',fontFamily:'monospace',fontSize:13}}/></label></>}
    <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}><button className="secondary-btn compact" onClick={onClose}>Cancel</button><button className="primary-btn compact" disabled={busy} onClick={()=>onSave(isLesson?{moduleId,title,lessonType,durationMinutes:duration,isPublished:published,content}:{courseId,title,description})}>{busy?'Saving…':'Save'}</button></div>
  </section></div>;
}

function AdminDashboard({ identity }: { identity:any }) {
  const [section,setSection]=useState<'overview'|'students'|'courses'|'enrollments'|'staff'|'pages'|'learning'>('overview');
  ...
  const [busy,setBusy]=useState(false),[error,setError]=useState(''),[courseForm,setCourseForm]=useState<any>(null),[pageForm,setPageForm]=useState<any>(null),[learningForm,setLearningForm]=useState<any>(null);
  const [enrollmentForm,setEnrollmentForm]=useState({studentId:'',courseId:''});
  const [pages,setPages]=useState<any[]>([]),[pageSearch,setPageSearch]=useState('');
  const [learning,setLearning]=useState<any>({courses:[],modules:[],lessons:[]});
  ...
  const loadPages=async(q='')=>{try{const r=await api.get('/api/admin/pages?search='+encodeURIComponent(q));setPages(r.data.pages||[])}catch(e:any){setError(e?.response?.data?.message||'Unable to load pages.')}};
  const loadLearning=async()=>{try{const r=await api.get('/api/admin/learning');setLearning(r.data||{courses:[],modules:[],lessons:[]})}catch(e:any){setError(e?.response?.data?.message||'Unable to load learning content.')}};
  ...
  const savePage=async(payload:any)=>{setBusy(true);setError('');try{if(pageForm?.id)await api.patch('/api/admin/pages/'+pageForm.id,payload);else await api.post('/api/admin/pages',payload);setPageForm(null);await loadPages(pageSearch)}catch(e:any){setError(e?.response?.data?.message||'Unable to save page.')}finally{setBusy(false)}};
  const saveLearning=async(payload:any)=>{setBusy(true);setError('');try{if(learningForm?.module_id)await api.patch('/api/admin/lessons/'+learningForm.id,payload);else if(learningForm?.id)await api.patch('/api/admin/modules/'+learningForm.id,payload);else if(payload.content)await api.post('/api/admin/lessons',payload);else await api.post('/api/admin/modules',payload);setLearningForm(null);await loadLearning()}catch(e:any){setError(e?.response?.data?.message||'Unable to save learning content.')}finally{setBusy(false)}};
  useEffect(()=>{loadOverview();loadStudents();loadCourses();loadEnrollments();loadStaff();loadPages();loadLearning()},[]);
  useEffect(()=>{const t=window.setTimeout(()=>{if(section==='students')loadStudents(search);if(section==='courses')loadCourses(courseSearch);if(section==='enrollments')loadEnrollments(enrollmentSearch);if(section==='staff')loadStaff(staffSearch,staffRole);if(section==='pages')loadPages(pageSearch)},250);return()=>window.clearTimeout(t)},[search,courseSearch,enrollmentSearch,staffSearch,staffRole,pageSearch,section]);
  ...
  return <main style={shell}><Nav kind="admin" onSignOut={signOut}/><div className="ops-wrap">
    ...
    <div className="ops-tabs">{[['overview','Dashboard'],['students','Students'],['courses','Courses'],['enrollments','Enrollments'],['staff','Staff'],['pages','Pages'],['learning','Learning']].map(([key,label])=><button key={key} className={section===key?'active':''} onClick={()=>setSection(key as any)}>{label}</button>)}</div>
    ...
    {section==='learning'&&<section>
      <div className="ops-student-toolbar"><div><h2>Learning Content</h2><p style={muted}>Build courses into modules and published lessons. Learners only receive published content.</p></div></div>
      <div className="ops-two"><section style={card}><h3>Modules</h3>{learning.modules.map((m:any)=><div className="ops-detail-row" key={m.id}><span><strong>{m.title}</strong><small>{learning.courses.find((c:any)=>c.id===m.course_id)?.title||'Course'} • {learning.lessons.filter((l:any)=>l.module_id===m.id).length} lessons</small></span><button className="secondary-btn compact" onClick={()=>setLearningForm(m)}>Edit</button></div>)}<button className="primary-btn compact" onClick={()=>setLearningForm({course_id:learning.courses[0]?.id||'',title:'',description:''})}>New module</button></section>
      <section style={card}><h3>Lessons</h3>{learning.lessons.map((l:any)=><div className="ops-detail-row" key={l.id}><span><strong>{l.title}</strong><small>{learning.modules.find((m:any)=>m.id===l.module_id)?.title||'Module'} • {l.lesson_type} • {l.is_published?'Published':'Draft'}</small></span><button className="secondary-btn compact" onClick={()=>setLearningForm(l)}>Edit</button></div>)}<button className="primary-btn compact" onClick={()=>setLearningForm({module_id:learning.modules[0]?.id||'',title:'',lesson_type:'lesson',duration_minutes:30,is_published:false,content:{blocks:[{type:'heading',text:'New lesson'},{type:'paragraph',text:'Add lesson content here.'}]}})}>New lesson</button></section></div>
      {learningForm&&<LearningContentEditor item={learningForm} modules={learning.modules} courses={learning.courses} onSave={saveLearning} onClose={()=>setLearningForm(null)} busy={busy}/>}
    </section>}
