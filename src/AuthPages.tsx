...
export function RegisterPage() {
  const [name, setName] = useState(''); const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [confirmPassword, setConfirmPassword] = useState(''); const [accepted, setAccepted] = useState(false); const [busy, setBusy] = useState(false); const [message, setMessage] = useState(''); const [created, setCreated] = useState(false);
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!name.trim() || !email.trim() || !password || !confirmPassword) { setMessage('Please complete all required fields.'); return; }
    ...
    setBusy(true);
    try {
      const { data } = await api.post('/api/auth/signup', { fullName: name.trim(), email: email.trim(), password });
      if (data?.user && !data?.needsEmailVerification) { window.location.replace(`${productionOrigin}/dashboard`); return; }
  ...
  return <AuthShell><div className="auth-card register-card"><div className="auth-card-icon"><GraduationCap size={24} /></div><span className="kicker">STUDENT REGISTRATION</span><h2>Create your learner account</h2><p className="auth-subtitle">Join IELTS Kenya Center to access courses, lessons, practice and measurable learning progress.</p><div className="auth-benefits"><span><Check size={15} /> Personal learner profile</span><span><Check size={15} /> Courses & lessons</span><span><Check size={15} /> Progress tracking</span></div><form onSubmit={submit} noValidate><label className="auth-field"><span>Full name <em>Required</em></span><div className="auth-input-wrap"><UserRound size={17} /><input name="fullName" value={name} onChange={e => setName(e.target.value)} autoComplete="name" placeholder="e.g. Jane Wanjiku" required /></div></label><label className="auth-field"><span>Email address <em>Required</em></span><div className="auth-input-wrap"><Mail size={17} /><input name="email" type="email" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" required /></div></label><PasswordField value={password} onChange={setPassword} autoComplete="new-password" label="Create password" /><div className="password-requirements" aria-live="polite"><span className={passwordChecks.length ? 'met' : ''}><Check size={13} /> 8+ characters</span><span className={passwordChecks.upper ? 'met' : ''}><Check size={13} /> Uppercase letter</span><span className={passwordChecks.number ? 'met' : ''}><Check size={13} /> Number</span></div><label className="auth-field"><span>Confirm password <em>Required</em></span><div className="auth-input-wrap"><LockKeyhole size={17} /><input name="confirmPassword" type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} autoComplete="new-password" placeholder="Re-enter your password" required /></div></label>{confirmPassword && <div className={`password-match ${password === confirmPassword ? 'match' : 'mismatch'}`}>{password === confirmPassword ? 'Passwords match.' : 'Passwords do not match yet.'}</div>}<label className="auth-consent"><input type="checkbox" checked={accepted} onChange={e => setAccepted(e.target.checked)} /><span>I agree to the learner terms and understand that I will need to verify my email before accessing my account.</span></label>{message && <div className="auth-error" role="alert">{message}</div>}<button className="auth-primary" disabled={busy}>{busy ? 'Creating your account…' : 'Create learner account'} <ArrowRight size={18} /></button></form><p className="auth-switch">Already have an account? <a href="/login">Sign in</a></p></div></AuthShell>;
}
