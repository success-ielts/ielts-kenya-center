...
function App() {
  ...
  return (
    <div className="site-shell">
      ...
      {opsOpen && <div className="ops-panel" role="dialog" aria-modal="true" aria-labelledby="ops-title"><div className="ops-card"><button className="modal-close" aria-label="Close operations panel" onClick={() => { window.location.hash = ''; setOpsOpen(false); }}><X /></button><span className="kicker">DEPLOYMENT OPERATIONS</span><h2 id="ops-title">Cloudflare DNS setup</h2><p>Securely configure the production apex record for this AppDeploy deployment.</p><form onSubmit={configureCloudflare}><label>Setup key<input type="password" value={opsKey} onChange={event => setOpsKey(event.target.value)} autoComplete="off" required /></label>{opsMessage && <div className="form-message" role="alert">{opsMessage}</div>}<button className="primary-btn full" disabled={opsBusy}>{opsBusy ? 'Configuring…' : 'Configure Cloudflare DNS'} <ArrowRight size={18} /></button><button type="button" className="secondary-btn full" disabled={emailTestBusy} onClick={testResendEmail}>{emailTestBusy ? 'Testing email…' : 'Test Resend email'}</button></form></div></div>}
