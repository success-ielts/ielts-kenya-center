...
\nasync function publicSeoShell(request: Request, env: Env, pageKey: string) {
  ...
  if (!html.includes('<h1>')) { const content=staticSeoBody[pageKey]; const body=content ? '<main><article><h1>'+escapeHtml(dbPage?.title||fallback?.[0]||title)+'</h1><p>'+escapeHtml(content.intro)+'</p>'+content.sections.map((s:any)=>'<section><h2>'+escapeHtml(s[0])+'</h2><ul>'+s[1].map((x:string)=>'<li>'+escapeHtml(x)+'</li>').join('')+'</ul></section>').join('')+'</article></main>' : '<main><article><h1>'+escapeHtml(dbPage?.title||fallback?.[0]||title)+'</h1><p>'+escapeHtml(dbPage?.body?.description||fallback?.[1]||description)+'</p></article></main>'; html=html.replace('<div id="root"></div>','<div id="root">'+body+'<section><h2>Related IELTS preparation</h2><ul>'+seoRelatedLinks.map((l:any)=>'<li><a href="'+l[1]+'">'+escapeHtml(l[0])+'</a></li>').join('')+'</ul></section></article></main></div>'); }
