...
const staticSeoFaqs:any={
'ielts-preparation-kenya':[['How should I start IELTS preparation in Kenya?','Start by identifying your IELTS type, current skill level and target band, then build a routine covering Listening, Reading, Writing and Speaking with regular review.'],['Does IELTS preparation cover all four skills?','Yes. A balanced preparation routine should practise Listening, Reading, Writing and Speaking, while giving extra time to skills that need improvement.']],
'academic-ielts':[['What is Academic IELTS preparation?','Academic IELTS preparation develops the four test skills while giving particular attention to Academic Reading and Academic Writing task requirements.'],['Can I prepare for Academic IELTS online?','Online preparation can provide structured lessons, practice and progress tracking when the learning routine is consistent and includes realistic timed practice.']],
'general-training-ielts':[['What is different about General Training IELTS?','Listening and Speaking use the same IELTS format as Academic IELTS, while Reading and Writing require preparation for General Training contexts and tasks.'],['How can I practise General Training IELTS?','Use General Training-focused Reading and Writing practice alongside regular Listening and Speaking work, then add timed practice as your confidence improves.']],
'ielts-listening':[['How can I improve IELTS Listening?','Practise predicting answers, recognising paraphrase, tracking speakers and checking spelling and details. Review mistakes after each practice set.'],['Should IELTS Listening practice be timed?','Untimed practice can help you learn a skill first. Add timed practice progressively so that you can apply the skill under realistic conditions.']],
'ielts-reading':[['How can I improve IELTS Reading?','Practise skimming for structure, scanning for locating signals, recognising paraphrase and checking the surrounding evidence before selecting an answer.'],['How should I review IELTS Reading mistakes?','Classify each mistake by vocabulary, misunderstanding, evidence location, question strategy or timing, then use the pattern to choose your next practice task.']],
'ielts-writing':[['What should I practise for IELTS Writing?','Practise understanding the prompt, planning relevant ideas, organising paragraphs, developing a clear response and checking vocabulary, grammar and spelling.'],['Is rewriting useful for IELTS Writing practice?','Yes. A plan-write-review-rewrite cycle can help identify recurring errors and turn feedback into specific improvements.']],
'ielts-speaking':[['How can I practise IELTS Speaking?','Practise answering directly, extending ideas with reasons and examples, and recording short responses so you can review pauses, repetition and clarity.'],['Should I memorise IELTS Speaking answers?','Flexible language practice is more useful than relying on memorised answers. Practise expressing the same idea in different ways.']],
'ielts-practice':[['How should I structure IELTS practice?','Use three stages: untimed learning, timed practice and mock-style practice. Review each attempt so it produces a clear next study action.'],['What is an IELTS error log?','An error log records recurring problems by skill or question type so that future study targets weaknesses instead of repeating only familiar tasks.']],
'ielts-courses':[['What IELTS courses are available?','Published courses include IELTS Foundations, Academic IELTS Band 7 Path, IELTS General Training Success, and Speaking & Writing Workshop.'],['Which IELTS course should I start with?','Choose a course based on your IELTS type, current level and preparation goal. Foundations is designed as a beginner starting point, while the other published courses target more specific needs.']],
'ielts-resources':[['What IELTS resources are available?','The resource hub covers Listening, Reading, Writing, Speaking, practice routines and structured IELTS courses.'],['How should I use IELTS study guides?','Choose one skill or problem at a time, apply the guidance in practice, review mistakes and return to the relevant guide when the same issue appears again.']]
};
...
async function publicSeoShell(request: Request, env: Env, pageKey: string) {
  ...
  if (!html.includes('<h1>')) { const content=staticSeoBody[pageKey]; const body=content ? '<main><article><h1>'+escapeHtml(dbPage?.title||fallback?.[0]||title)+'</h1><p>'+escapeHtml(content.intro)+'</p>'+content.sections.map((s:any)=>'<section><h2>'+escapeHtml(s[0])+'</h2><ul>'+s[1].map((x:string)=>'<li>'+escapeHtml(x)+'</li>').join('')+'</ul></section>').join('') : '<main><article><h1>'+escapeHtml(dbPage?.title||fallback?.[0]||title)+'</h1><p>'+escapeHtml(dbPage?.body?.description||fallback?.[1]||description)+'</p>'; const faqHtml=(staticSeoFaqs[pageKey]||[]).length?'<section><h2>Frequently asked questions</h2>'+staticSeoFaqs[pageKey].map((f:any)=>'<article><h3>'+escapeHtml(f[0])+'</h3><p>'+escapeHtml(f[1])+'</p></article>').join('')+'</section>':''; const related=faqHtml+'<section><h2>Related IELTS preparation</h2><ul>'+seoRelatedLinks.map((l:any)=>'<li><a href="'+l[1]+'">'+escapeHtml(l[0])+'</a></li>').join('')+'</ul></section></article></main>'; html=html.replace('<div id="root"></div>','<div id="root">'+body+related+'</div>'); }
