...
function CourseView({ courseId }: { courseId:string }) {
  ...
  if(error)return <main className="learner-shell"><section className="learner-hero"><div><span className="kicker">COURSE</span><h1>Access unavailable</h1><p>{error}</p><a className="secondary-btn" href="/dashboard">Return to dashboard</a></div></section></main>;
  if(!data)return <main className="learner-shell"><section style={{padding:40}}>Loading course…</section></main>;
  const progressMap=new Map((data.progress||[]).map((p:any)=>[p.lesson_id,p]));
  return <main className="learner-shell"><section className="learner-hero"><div><span className="kicker">{String(data.course?.ielts_type||'IELTS').replaceAll('_',' ').toUpperCase()}</span><h1>{data.course?.title}</h1><p>{data.course?.description}</p></div><div className="learner-actions"><strong>{data.courseProgress}% complete</strong><a className="secondary-btn" href="/dashboard">Dashboard</a></div></section><section className="learner-section"><div className="section-heading"><div><span className="kicker">COURSE STRUCTURE</span><h2>Work through the modules in order.</h2></div><p>Open a published lesson to study and save your progress.</p></div>{(data.modules||[]).map((m:any)=><article style={{...card,marginBottom:14}} key={m.id}><span className="kicker">MODULE {m.sort_order+1}</span><h3>{m.title}</h3><p>{m.description}</p><div>{(data.lessons||[]).filter((l:any)=>l.module_id===m.id).map((l:any)=><a key={l.id} href={'/learn/lesson/'+encodeURIComponent(l.id)} className="ops-detail-row" style={{display:'flex',textDecoration:'none',color:'inherit',marginTop:8}}><span><strong>{l.title}</strong><small>{l.lesson_type} • {l.duration_minutes||'—'} min</small></span><span>{progressMap.get(l.id)?.status==='completed'?'✓ Completed':'Open'} <ArrowRight size={16}/></span></a>)}</div></article>)}</section></main>;
}
