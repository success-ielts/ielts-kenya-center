...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/signup') {
      ...
      const email = String(body.email).trim(); const fullName = String(body.fullName).trim();
      const generated = await adminGenerateLink(env, { type: 'signup', email, password: String(body.password), data: { full_name: fullName }, redirect_to: origin });
    ...
    if (request.method === 'POST' && url.pathname === '/api/auth/recover') {
      ...
      const generated = await adminGenerateLink(env, { type: 'recovery', email, redirect_to: origin });
      if (generated.response.ok) {
        const actionLink = generated.data?.properties?.action_link || generated.data?.action_link;
        if (actionLink) ctx.waitUntil(sendAuthEmail(env, email, 'Reset your IELTS Kenya Center password', 'Reset your IELTS Kenya Center password', 'We received a request to change the password for your learner account. Use the secure link below to choose a new password.', actionLink, 'Choose a new password'));
      }
