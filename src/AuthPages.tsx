...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setMessage(''); if (!email.trim() || !password) { setMessage('Email and password are required.'); return; } setBusy(true); try { const { data } = await api.post('/api/auth/signin', { email: email.trim(), password }); const destination = destinationFor(data?.user); window.location.replace(`${productionOrigin}${destination}`); } catch (err: any) { setMessage(mapAuthError(err)); } finally { setBusy(false); } };
  const google = async () => { setMessage(''); setGoogleBusy(true); try { window.location.assign(`${productionOrigin}/api/auth/google`); } catch { setMessage('Google sign-in could not be started. Please use email and password.'); setGoogleBusy(false); } };
